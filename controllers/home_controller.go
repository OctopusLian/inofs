...
func (h *HomeController) Get() {
	...
	for _, auth_data := range auths { // 一级菜单
		...
		tree_data := models.Tree{Id: auth_data.Id, AuthName: auth_data.AuthName, UrlFor: auth_data.UrlFor, Weight: auth_data.Weight, Children: []*models.Tree{}}
	...
	for _, apply := range cars_apply {
		...
		if ret > 0 { // 已经逾期
			message_notify := models.MessageNotify{
				Flag:    1,
				Title:   "车辆归还逾期",
				Content: content,
				User:    &user,
...
// 递归
func GetChildNode(pid int, treenode *models.Tree) {
	...
	for i := 0; i < len(auths); i++ {
		...
		tree_data := models.Tree{Id: auths[i].Id, AuthName: auths[i].AuthName, UrlFor: auths[i].UrlFor, Weight: auths[i].Weight, Children: []*models.Tree{}}
