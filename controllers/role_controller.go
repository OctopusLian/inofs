...
func (r *RoleController) DoAdd() {
	...
	role := models.Role{RoleName: role_name, Desc: desc, IsActive: is_active, CreateTime: time.Now()}
	o := orm.NewOrm()
	_, err := o.Insert(&role)
...
func (r *RoleController) DoRoleUser() {
	...
	for _, user_id := range user_id_arr {
		user := models.User{Id: utils.StrToInt(user_id)}
		m2m := o.QueryM2M(&role, "User")
		m2m.Add(user)
...
func (r *RoleController) DoRoleAuth() {
	...
	for _, auth_id := range id_arr {
		...
		if auth_id_int != 0 {
			auth_data := models.Auth{Id: auth_id_int}
			m2m := o.QueryM2M(&role, "Auth")
			m2m.Add(auth_data)
		}
